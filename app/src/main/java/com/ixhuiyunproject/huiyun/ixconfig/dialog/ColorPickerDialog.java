...
/* 使用方法：dialog = new ColorPickerDialog(context, tvText.getTextColors().getDefaultColor(), 
...
public class ColorPickerDialog extends Dialog {
    ...
    public ColorPickerDialog(Context context, String title, 
    	...
    	this(context, StaticValue.COLOR, title, listener);
    }
    ...
    private class ColorPickerView extends View {
    	...
    	private Paint mCenterPaint;//中间上半圆画笔
    	private Paint mOffPaint;//中间下半圆画笔
    	private Paint mLinePaint;//分隔线画笔
    	private Paint mRectPaint;//渐变方块画笔
    	private Paint mTextPaint;//文字画笔
    	private Paint mTitleLinePaint;//标题分割线画笔
    	...
    	private boolean highlightCenterOff;
    	
    	private RectF rfMiddle;
    	private RectF rfLoop;
		...
		public ColorPickerView(Context context, int height, int width) {
            ...
            mOffPaint.setColor(Color.BLACK);
            mOffPaint.setStrokeWidth(5);
            centerRadius = (r - mPaint.getStrokeWidth() / 2 ) * 0.7f;
            rfMiddle = new RectF(-centerRadius,-centerRadius,centerRadius,centerRadius);
            ...
            mTitleLinePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
            mTitleLinePaint.setColor(Color.parseColor("#28A465"));
            mTitleLinePaint.setStrokeWidth(4);
		...
		protected void onDraw(Canvas canvas) {
            ...
            //标题线
            canvas.drawLine( -mWidth / 2 * 0.8f, -mHeight / 2 + 50, 
            		mWidth / 2 * 0.8f, -mHeight / 2 + 50, mTitleLinePaint);
            //画中心圆
            canvas.drawArc(rfMiddle, 0, 180, true, mOffPaint);
            canvas.drawArc(rfMiddle, 180, 180, true, mCenterPaint);
            canvas.drawText("OFF", 0, centerRadius * 0.6f, mTextPaint);
            canvas.drawText("ON", 0, -centerRadius * 0.2f, mTextPaint);
		...
		public boolean onTouchEvent(MotionEvent event) {
            ...
            boolean inCenterUp = inCenterUp(x, y, centerRadius);
            boolean inCenterDown = inCenterDown(x, y, centerRadius);
            ...
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                	...
                	downInRect = inRect;
                	highlightCenter = inCenterUp;
                	highlightCenterOff = inCenterDown;
                case MotionEvent.ACTION_MOVE:
                	...
                	if(debug) Log.v(TAG, "[MOVE] 高亮: " + highlightCenter + "微亮: " + highlightCenterLittle + " 中心: " + inCenterUp);
                	if((highlightCenter && inCenterUp) || (highlightCenterLittle && inCenterUp)) {//点击中心圆, 当前移动在中心圆
                ...
                case MotionEvent.ACTION_UP:
                	if(highlightCenter && inCenterUp) {//点击在中心圆, 且当前启动在中心圆
                		if(mListener != null) {
                			mListener.colorChanged(mCenterPaint.getColor());
                			if(bCloseWhenTouchCenter){
                				ColorPickerDialog.this.dismiss();
                			}
                		}
                	} else if(highlightCenterOff && inCenterDown){
                		if(mListener != null) {
                	...
                	if(highlightCenterOff){
                		highlightCenterOff = false;
                	}
                	invalidate();
                    break;
            }
		 ...
		 * @return
		 */
		private boolean inCenterUp(float x, float y, float centerRadius) {
		 ...
		 * @return
		 */
		private boolean inCenterDown(float x, float y, float centerRadius){
			double centerCircle = Math.PI * centerRadius * centerRadius;
			double fingerCircle = Math.PI * (x * x + y * y);
			if(fingerCircle < centerCircle) {
				if(y >= 0){
	...
	private boolean bCloseWhenTouchCenter = false;
	/** 设置点击中间后是否会自己关闭对话框
	 * @param b
	 */
	public void enableCenterClose(boolean b){
		bCloseWhenTouchCenter = b;
	}
}
